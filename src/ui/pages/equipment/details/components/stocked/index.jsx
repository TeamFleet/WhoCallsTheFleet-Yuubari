...
const EquipmentDetailsComponentStocked = extend({
    ...
    ({ className, equipment }) => {
        ...
        return (
            <ComponentContainer className={className} title={__("equipment_details.stocked")}>
                {!!stockedOn.length && <List className={className} stockedOn={stockedOn} />}
                {!stockedOn.length && <span className="disabled">{__("none")}</span>}
                {/* <ListShips
