...
const EquipmentDetailsComponentImprovements = extend({
    ...
    ({ className, equipment }) => {
        ...
        const upgradable = (Array.isArray(equipment.upgrade_to) && equipment.upgrade_to.length)
        return (
            <ComponentContainer className={className} title={__("equipment_details.improvements")}>
                {hasItem && <div className="list">
                    {list.map((data, index) => (
                        <EquipmentDetailsComponentImprovementsImprovement
                            data={data} key={index} upgradable={upgradable} className="flex-item" />
                    ))}
...
const EquipmentDetailsComponentImprovementsImprovement = extend({
    ...
    ({
        className, upgradable,
        data: {
            upgrade, req, resource
        }
    }) =>
