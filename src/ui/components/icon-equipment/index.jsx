...
const IconEquipment = extend({
    ...
    memo(({ tag, icon, type, equipment, children, ...props }) => {
        ...
        if (Array.isArray(matches)) {
            ...
            else if (matches[2]) suffix = `${matches[2]}+`;
            else if (matches[3]) suffix = `0~${matches[3]}+`;
            if (suffix) props['data-suffix-type'] = 'stat-range';
        }
        ...
        if (Array.isArray(matches)) {
            ...
            if (suffix) props['data-suffix-type'] = 'equipment-name';
        }
        ...
        if (isNaN(iconId)) iconId = undefined;

        if (!suffix)
            suffix = ('' + icon).replace(iconId, '').toUpperCase() || undefined;

        return (
            <Component data-icon={iconId} data-suffix={suffix} {...props}>
                {children}
            </Component>
        );
    })
