...
const requiredEquipmentSamples = {
    ...
    hasAircraftHasStat_asw_7: [82, 83, 69, 244],
...
class CalculatorLevelOASW extends Component {
    constructor(props) {
        super(props);
        ...
        this.canOASW =
            this.canAlways ||
            (Array.isArray(this.oaswTable) && this.oaswTable.length)
                ? true
                : false;
            ...
            this.oaswTable.forEach((OASW) => {
                if (OASW.equipments)
                    for (let req in OASW.equipments) {
                        let hasStat;
                        if (
                            typeof OASW.equipments[req] === 'object' &&
                            OASW.equipments[req].hasStat
                        ) {
                        ...
                        if (typeof value === 'string') {
                            ...
                            Array.isArray(
                                requiredEquipmentSamples[
                                    `${req}${OASW.equipments[req]}`
                                ]
                            )
                        ) {
                            if (
        ...
        equipmentList
            ...
            .forEach((equipmentId) => {
                if (this.equipmentList.includes(equipmentId)) return false;
    ...
    update(id, count) {
        if (this.state[id] !== count) {
            this.setState((prevState, props) => {
                ...
                prevState[id] = count;
                const equipments = [];
                for (const equipmentID in prevState) {
                ...
                const result = kckit.calculate.ship.levelOASW(
                    props.ship,
                    equipments
                );
                const meetEquipmentsRequirement = kckit.check.oasw(
                    props.ship,
                    equipments
                );
                if (__DEV__)
                    // eslint-disable-next-line no-console
                    console.log(equipments, result, meetEquipmentsRequirement);
                return {
                    [id]: count,
                    result,
        ...
        if (Array.isArray(id)) {
            if (Array.isArray(id[2]))
                displayName =
                    ...
                    ')';
            else displayName = __(`equipment_types`, id[0]);
            id = id[1];
        ...
        return (
            <Equipment
                ...
                className={classNames({
                    'is-required':
                        !this.state.meetEquipmentsRequirement &&
                        this.isRequired.includes(id),
    ...
    render() {
        ...
        return (
            ...
            >
                ...
                {!this.statUnknown &&
                    !this.canOASW &&
                    this.props.componentUnable}
                {!this.statUnknown &&
                    this.canOASW &&
                    this.canAlways &&
                    this.props.componentAlways}

                {this.canOASW && !this.canAlways && this.renderEquipmentGroup()}
                {this.canOASW && !this.canAlways && (
                    <div className="area-result">
                        {this.state.meetEquipmentsRequirement &&
                            __('oasw_calculator.result')}
                        {this.getResult()}
                    </div>
                )}
            </div>
