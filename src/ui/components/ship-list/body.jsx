...
class ShipListView extends React.Component {
    ...
    constructor(props) {
        super(props);
        if (!props.isInit) {
            props.dispatch(shipListInit(props.id));
        }
    }
    ...
    */
    ...
    render() {
        ...
        if (this.props.isModeCompare && this.props.compareState === 'comparing')
            ...
            showType = 'filtered';
        else if (__CLIENT__) showType = 'collection';
        ...
        return (
            ...
            >
                ...
                <BodyFiltered
                    show={showType === 'filtered'}
                    id={this.props.id}
                />
                {__CLIENT__ && (
                    <BodyCollections
                        show={showType === 'collection'}
                        id={this.props.id}
                    />
                )}
                {__SERVER__ &&
...
const BodyCompare = extend({
    connect: (state, ownProps) => {
        const { compareState, compareList = [] } =
            state.shipList[ownProps.id] || {};
        return {
            compareState,
            compareList,
        ...
        return (
            <CSSTransitionGroup>
                {collection.list.map((type, index2) => {
                    ...
                    return (
                        <CSSTransitionComponent
                            key={collection.name + index2}
                            data-key={collection.name + index2}
                        >
                            <div
                                className={classNames({
                                    first: index2 === 0,
