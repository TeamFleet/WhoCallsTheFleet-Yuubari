...
class DayAndShip extends React.Component {
        ...
        return (
            <div className="item" key={index}>
                <div className="days">
                    {days.map((day, index) => (
                        <span
                            className={'day' + (dataDays[index] ? ' on' : '')}
                            key={index}
                        >
                            {__(`day_abbr`, day)}
                        </span>
                ...
                <div className={'ships' + (!dataShips ? ' is-any' : '')}>
                    {dataShips &&
                        dataShips.map((ship, index) => (
                            <Link
                                className="ship color-alt"
                                key={index}
    ...
    render() {
        ...
        if (typeof data === 'object' && data.req) data = data.req;
...
class _Resources extends React.Component {
    constructor(props) {
        ...
        if (typeof this.data === 'object' && this.data.resource) {
            if (
                typeof this.upgradable === 'undefined' &&
                Array.isArray(this.data.upgrade) &&
                this.data.upgrade.length
            )
    ...
    render() {
        return (
            <div className={this.props.className}>
                {this.renderCategory(
                    'resources',
                    this.data[0] || [undefined, undefined, undefined, undefined]
                )}
                {this.renderCategory(
                    0,
                    this.data[1] || [
                ...
                {this.upgradable &&
                    this.renderCategory('upgrading', this.data[3])}
