...
const equipmentCollectionsPretty = [];
const equipmentTypesAppended = [];
        ...
        async (type) =>
            new Promise(async function (resolve /*, reject*/) {
                ...
                equipments.forEach((equipment) => {
                    const l =
                        equipmentCollections[index].list[thisSubIndex]
                            .equipments;
...
module.exports = async (dbpath, topath) => {
    ...
    const equipmentTypeCollections = await db.equipmentTypeCollections
        .cfind({})
        .sort({ id: 1 })
    ...
    const equipmentsRemains = await db.equipments
        .cfind({
    ...
    equipmentCollections = equipmentCollections.filter((collection) => {
        collection.list = collection.list.filter(
            (types) => types.equipments.length > 0
        );
    ...
    equipmentCollections.forEach((collection, indexCollection) => {
        ...
        collection.list.forEach((equipmentType, indexEquipmentType) => {
            equipmentCollectionsPretty[indexCollection].list[
                indexEquipmentType
            ] = {
                type: `[${equipmentType.type}]`,
                equipments: equipmentType.equipments.map(
                    (equipment) => `[${equipment.id}] ${equipment.name.ja_jp}`
                ),
            };
